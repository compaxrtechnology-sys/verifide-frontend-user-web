  ...
  const handleJobAction = async (action, job) => {
    ...
    if (action === "edit") {
      ...
      try {
        ...
        const responsex = await dispatch(
          recommendedApplicationList({ job_id: job?._id }),
        ).unwrap();

        console.log("This is recommended response", responsex);
        setRecommendedUsers(responsex?.data?.list || []);
  ...
  const handleAction = async (type) => {
    ...
    try {
      const res = await dispatch(
        updateApplicationStatus({
          status: type,
          application_ids: selectedIds, // ✅ already array
        }),
      ).unwrap();
      ...
      setSelectedJob(result?.data?.list || []);
      setSelectedIds([]); // ✅ reset array
    } catch (error) {
      console.error("Error updating application status:", error);
      toast.error(error?.message || "Something went wrong.");
    }
  };
  const handleActionRecommended = async (type) => {
    if (
      !Array.isArray(selectedRecommendedIds) ||
      selectedRecommendedIds.length === 0
    ) {
      toast.error("Please select at least one applicant.");
      return;
    }
    ...
    try {
      const res = await dispatch(
        updateApplicationStatus({
          status: type,
          recommended_user_ids: selectedRecommendedIds, // ✅ already array
          job_id: jobId,
        }),
      ).unwrap();
      ...
      return (
        <div className="w-full">
          ...
          <div className="flex justify-center mb-2 mt-2">
            <CustomButton variant="primary" onClick={handleOpenRecommended}>
              View Recommended
            </CustomButton>
          </div>
        </div>
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      const res = await dispatch(
        updateApplicationStatus({
          status: "REJECTED",
          application_ids: [showRejectModalData?._id], // ✅ single wrapped in array
          remarks: rejectReason,
        }),
      ).unwrap();
        ...
        <div className="flex justify-between md:flex-row flex-col mb-6 space-y-4 lg:space-y-0">
          ...
          <div className="flex  items-center gap-3">
            ...
            <button
              ...
              className="relative p-2 border border-gray-300 rounded-lg hover:glassy-card focus:ring-2 focus:ring-blue-500 focus:ransparent outline-none"
            >
  ...
  />
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <CustomInput
                      type="checkbox"
                      checked={selectedRecommendedIds.includes(user._id)}
                      onChange={() =>
        ...
        {recommendedUsers.length > 0 && (
          <div className="flex justify-end gap-2 mt-4  pt-3">
            <CustomButton
              variant="outline"
              onClick={() => handleActionRecommended("REJECTED")}
              disabled={!selectedRecommendedIds.length}
            >
              Reject
            </CustomButton>
            ...
            <CustomButton
              variant="primary"
              onClick={() => handleActionRecommended("SHORTLISTED")}
              disabled={!selectedRecommendedIds.length}
            >
              Select
            </CustomButton>
