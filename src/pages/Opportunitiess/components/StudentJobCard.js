...
const JobCard = ({
  ...
  const formatSalaryRange = () => {
    ...
    return `₹${parseInt(salary_range).toLocaleString()} / month`;
  };
  ...
  const getApplicationStatus = () => {
    ...
    switch (status) {
      case "applied":
        return {
          text: "Applied",
          color: "glassy-card text-blue-600 border-blue-200",
        };
      case "shortlisted":
        return {
          text: "Shortlisted",
          color: "glassy-card text-green-600 border-green-200",
        };
    ...
    switch (status) {
      case "scheduled":
        return {
          ...
          color: "bg-purple-50 glassy-text-primary border-purple-200",
        };
      case "completed":
        return {
          text: "Interview Completed",
          color: "glassy-card glassy-text-secondary border-gray-200",
        };
      case "cancelled":
        return {
  ...
  return (
    <div
      ...
      } rounded-2xl p-6 flex flex-col space-y-4 w-full h-full transition-all hover:shadow-lg cursor-pointer`}
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 text-sm  ">
        {[job_type, job_location, pay_type ,degree, field_of_studies ].map((item, idx) => (
          <span
            key={idx}
