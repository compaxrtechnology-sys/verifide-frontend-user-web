...
const AnswerCard = ({
    ...
    if (questionType === "single_choice") {
      return (
        <div className="space-y-2">
          ...
          <div>
            <p className="text-xs font-medium glassy-text-primary">
              {" "}
              Selected Answer:
            </p>
            <p className="text-sm   py-1.5 rounded glassy-text-primary">
          ...
          <div>
            ...
            <ul className="list-disc pl-5">
              {selectedOptions.map((option, i) => (
                <li key={i} className="text-sm glassy-text-primary">
                  {option}
                </li>
              ))}
            </ul>
  ...
  return (
    <div className="glassy-card rounded-xl border space-y-2 mb-4 last:mb-0">
      <p className="text-sm font-medium glassy-text-primary p-3 break-words break-all whitespace-pre-wrap">
        {index + 1}. {question}
      </p>

      <div className="mt-2 p-3 break-words break-all whitespace-pre-wrap">
        {renderAnswer()}
      </div>
    </div>
  );
