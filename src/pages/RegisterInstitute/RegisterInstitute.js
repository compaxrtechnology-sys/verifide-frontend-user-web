  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // } else
      {
        ...
        const newCompanyId = res?.data?._id || res?.data?.company?._id;

        if (newCompanyId) {
          await switchAccountFunction(newCompanyId, "institution");
        } else {
          // fallback
          navigate("/user/feed");
        }
  ...
  const switchAccountFunction = async (prefillId, type) => {
    try {
      ...
      if (res) {
        ...
        setCookie("ROLE", res.data.accessMode);
        setCookie("ACTIVE_MODE", type);
