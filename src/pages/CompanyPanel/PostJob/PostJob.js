...
const PostJob = () => {
  ...
  const degreeList = arrayTransform(
   ...
   const fieldsOfStudyList = arrayTransform(
    degreeSelector?.getAllFieldsOfStudyData?.data?.data?.list,
  );
  console.log("this is the degree and field of study list", degreeList, fieldsOfStudyList);
  ...
  const fetchJobData = useCallback(async () => {
    try {
      ...
      setFormData({
        ...
        degree_id: res?.data?.degree_id,
        field_of_studies: res?.data?.field_of_studies,
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      if (!formData.degree_id)
        newErrors.degree_id = "Field of study is required";
      if (!formData.field_of_studies)
        newErrors.field_of_studies = "Degree is required";
...
const handleAddItem = async () => {
  try {
    ...
    switch (addModalState.type) {
      ...
      case "degree":
        updateAction = updateDegreeData; // <-- existing redux update
        apiAction = degreeAddOnData;
        break;

      case "field":
        updateAction = updateFieldsOfStudyData; // <-- existing redux update
        apiAction = fieldsOfStudyAddOnData;
        break;
    ...
    // 🔥 API CALL
    const payload =
      addModalState.type === "degree" || addModalState.type === "field"
        ? inputFields
        : { type, ...inputFields };

    const res = await dispatch(apiAction(payload)).unwrap();
    ...
    // ✅ Update redux list (dropdown options)
    dispatch(updateAction(newItem));

    // ✅ Update local form instantly
    if (selectField === "required_skills") {
      const newSkillOption = {
        value: newItem._id,
        label: newItem.name,
  ...
  const renderStep1 = () => (
    <div className="space-y-3">
      <StepFirst
        ...
        fieldsOfStudyList={fieldsOfStudyList}
        formData={formData}
