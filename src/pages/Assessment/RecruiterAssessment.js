...
const AssessmentCard = ({ assessment, onEdit, onDelete, onStatusChange }) => {
  ...
  const visibleSkills = expandedSkills
    ...
    : assessment.skill_ids.slice(0, MAX_VISIBLE_SKILLS);
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <span className="flex items-center text-xs space-x-1  border border-gray-200 rounded-full px-3 py-1.5 glassy-card">
          ...
          <span className="glassy-text-secondary">
            {assessment.no_of_questions} Questions
          </span>
        </span>
        <span className="flex items-center text-xs space-x-1  border border-gray-200 rounded-full px-3 py-1.5 glassy-card">
...
const RecruiterAssessment = () => {
  ...
  const skillList = arrayTransform(
    selector2?.masterSkillsData?.data?.data?.list,
  );
  const levelList = arrayTransform(
    selector2?.masterLevelData?.data?.data?.list,
  );
  ...
  const fetchAssessmentsList = useCallback(
    async (page = 1, search = "") => {
      const apiPayload = {
        page,
        size: postsPerPage,
        keyWord: search,
        select:
          "title description skill_ids level_id no_of_questions isDisable updatedAt",
  ...
  const validateForm = useCallback(() => {
    ...
    //   newErrors.material_url = "Material URL is required";
    // } else {
    //   try {
    //     new URL(formData.material_url);
    //   } catch {
    //     newErrors.material_url = "Enter a valid URL (YouTube or course link)";
    //   }
    // }

    if (
      typeof formData.no_of_questions !== "number" ||
      formData.no_of_questions <= 0
    ) {
      newErrors.no_of_questions = "Number of questions must be greater than 0";
    }
    if (
      typeof formData.max_attempts !== "number" ||
    ...
    if (formData.max_attempts < formData.no_of_questions) {
      newErrors.max_attempts =
        "Max attempts should not be less than number of questions";
    }
    if (
      !Array.isArray(formData.questions) ||
      formData.questions.length !== formData.no_of_questions
    ) {
      ...
      try {
        ...
        setFormData({
          ...
          questions: res.data.questions.map((question) => ({
            ...
            option_format: optionFormats.find(
              (of) => of.value === question.option_format,
            ) || {
              value: question.option_format,
              label:
                question.option_format.charAt(0).toUpperCase() +
                question.option_format.slice(1),
            },
          })),
        });
        setIsModalOpen(true);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // console.log("this is the validatoin errors", validationErrors);
    // if (Object.keys(validationErrors).length > 0) {
    ...
    const totalTime = formData.questions.reduce(
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const res = await dispatch(
        assessmentsDelete({ _id: modalData.data }),
      ).unwrap();
      toast.success(res?.message);
      await fetchAssessmentsList(currentPage, searchTerm);
    } catch (error) {
  ...
  const addOption = () => {
    ...
    if (
      questionFormData.options.includes(questionFormData.currentOption.trim())
    ) {
      setQuestionErrors((prev) => ({
        ...prev,
  ...
  const removeOption = (index) => {
    ...
    setQuestionFormData((prev) => ({
      ...
      correct_options: prev.correct_options.filter(
  ...
  const selectJson = useMemo(
    () => ({
      ...
      levels:
        levels?.map((level) => ({ value: level._id, label: level.name })) || [],
    }),
    [skills, levels],
  );
  ...
  return (
    <div className=" min-h-screen px-4 lg:px-6 md:py-6 py-2">
      <div className="w-full mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="glassy-card rounded-xl shadow-sm border border-gray-200 p-8 text-center">
            ...
            <h3 className="text-lg font-medium glassy-text-primary mb-2">
      ...
      <Modal
        ...
        title={
          modalData.type === "edit" ? "Edit Assessment" : "Add New Assessment"
        }
      >
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <CustomInput
                ...
                onChange={(e) =>
                  handleChange("no_of_questions", parseInt(e.target.value) || 0)
                }
              ...
              <CustomInput
                ...
                name="max_attempts"
                onChange={(e) =>
                  handleChange("max_attempts", parseInt(e.target.value) || 1)
                }
      ...
      <Modal
        ...
        title={
          questionModalData.type === "edit" ? "Edit Question" : "Add Question"
        }
        icon={
          questionModalData.type === "edit" ? (
        ...
        buttonLabel={
          questionModalData.type === "edit" ? "Update Question" : "Add Question"
        }
      >
              ...
              <div className="flex gap-3 justify-start items-center">
                <div className="flex-1">
                  <CustomInput
                    ...
                    onChange={(e) =>
                      handleQuestionChange("currentOption", e.target.value)
                    }
                  ...
                  {questionFormData.options.map((opt, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1">
                        <input
                          type={
                            questionFormData.question_type?.value ===
                            "single_choice"
                              ? "radio"
                              : "checkbox"
                          }
                          checked={questionFormData.correct_options.includes(
                            opt,
                          )}
